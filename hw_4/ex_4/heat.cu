...
int main(int argc, char **argv) {
  ...
  gpuCheck(cudaMallocManaged(&tmp,dimX*sizeof(double)) != cudaSuccess);
  gpuCheck(cudaMallocManaged(&temp,dimX*sizeof(double)) != cudaSuccess);

  gpuCheck(cudaMallocManaged(&ARowPtr,(dimX+1)*sizeof(int)) != cudaSuccess);
  gpuCheck(cudaMallocManaged(&AColIndx,nzv*sizeof(int)) != cudaSuccess);

  gpuCheck(cudaMallocManaged(&A,nzv*sizeof(double)) != cudaSuccess);
  ...
  cublasSetPointerMode(cublasHandle, CUSPARSE_POINTER_MODE_HOST);

  //@@ Insert code to call cusparse api to create the mat descriptor used by cuSPARSE
  cusparseCreateMatDescr(&Adescriptor);

    //https://github.com/rmaksymiuk/DD2360HT22/blob/main/hw_4/ex_4/Assignment4_Ex4.ipynb
